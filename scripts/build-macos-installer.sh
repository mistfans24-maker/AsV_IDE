...
rsync -a --delete --exclude='.env*' --exclude='.git' --exclude='outputs' --exclude='work' --exclude='docs' --exclude='node_modules/.cache' "$ROOT/" "$RUNTIME/"
rm -rf "$STAGE"
mkdir -p "$PACKAGE_DIR"
pkgbuild --component "$APP" --install-location /Applications --identifier local.asvide.app --version 0.1.0 "$PACKAGE_DIR/AsV_IDE.pkg"
