...
final class ExecutorBridge: NSObject, WKScriptMessageHandler {
  ...
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
    ...
    let commands = ["Python": ("python3", "py"), "Ruby": ("ruby", "rb"), "Lua": ("lua", "lua")]
...
final class AppDelegate: NSObject, NSApplicationDelegate {
  private var server: Process?; private let bridge = VaultBridge(); private let executor = ExecutorBridge()
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    let content = WKUserContentController(); content.add(bridge, name: "asvVault"); content.add(executor, name: "asvExecute")
