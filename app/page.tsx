...
function NotebookPage({background,onBack}:{background:string;onBack:()=>void}) {
  ...
  return <main className={`notebook theme-${background}`}><header className="notebook-top"><div className="brand"><img src="/asv-logo.png" alt="AsV_IDE"/><em>NOTEBOOK · LOCAL COMPUTE</em></div><div><button className="visual-button" onClick={onBack}>← IDE</button><button className="run-button" onClick={run}>▶ Run cell</button></div></header><section className="notebook-sheet"><div className="notebook-title"><span>LOCAL NOTEBOOK</span><h1>Untitled experiment</h1><p>Code, output, and local servers—like a notebook, without cloud compute.</p></div><article className="note-cell markdown"><b># Build something local</b><p>Run Python or JavaScript cells. Start a server from your own code when you are ready.</p></article><article className="note-cell code-cell"><div className="cell-head"><span>⌘ CODE</span><select value={language} onChange={e=>setLanguage(e.target.value)}><option>Python</option><option>JavaScript</option><option>Ruby</option><option>Lua</option></select><button onClick={run}>▶</button></div><CodeEditor value={code} language={language} onChange={setCode}/><pre>{output}</pre></article><article className="note-cell server-cell"><div><span>◉ LOCALHOST</span><h2>Start server from this code</h2><p>Your code must create a server on the selected port. It stays on this computer.</p></div><input value={port} onChange={e=>setPort(e.target.value.replace(/[^0-9]/g,""))} aria-label="Local port"/><button className="primary" onClick={start}>Start localhost</button>{url&&<a href={url} target="_blank" rel="noreferrer">Open {url} ↗</a>}</article></section></main>;
}
